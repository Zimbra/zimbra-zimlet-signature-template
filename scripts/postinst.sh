...
if [ $? -ne 0 ]; then
   ...
   echo "*** zmzimletctl -l deploy /opt/zimbra/zimlets-network/zimbra-zimlet-signature-template.zip ***"
   echo "*** zmprov fc zimlet ***"
else
   echo "*** Deploying Zimbra-Zimlet-Signature-Template  zimlet ***"
   su - zimbra -c  "zmzimletctl -l deploy /opt/zimbra/zimlets-network/zimbra-zimlet-signature-template.zip"
